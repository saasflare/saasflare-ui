...
// Swatch fallbacks — mirror `:root[data-palette="…"] { --primary-l … }` and
// the `.dark` overrides in packages/ui/styles/palettes.css. Used on SSR / first
// client paint before the runtime reader below picks the live values from the
// CSS vars. The dark/light split matters for achromatic palettes that *invert*
// across modes (notably `black`: `0` in light, `1` in dark — pure black vs
// pure white). Keep both maps in lockstep with palettes.css.
// Pastel sweep used as the `colorful` swatch — mirrors --colorful-gradient
// in palettes.css. Lives here too so the swatch can render the gradient
// at first paint without depending on a live CSS-var read.
const COLORFUL_SWATCH_GRADIENT =
    "linear-gradient(95deg, #ffd4a3 0%, #ffb8c5 35%, #d4bce8 65%, #b8d8e5 100%)"

const PALETTE_SWATCH_FALLBACK_DARK: Record<(typeof PALETTES)[number], string> = {
...
const PALETTE_SWATCH_FALLBACK_LIGHT: Record<(typeof PALETTES)[number], string> = {
...
/**
 * Read the actual `--primary-h / --primary-c / --primary-l` triple every preset
 * palette resolves to under the current `.dark` / `.light` class, by briefly
 * swapping `<html data-palette>` and reading the computed style. All swaps
 * happen synchronously inside one JS turn, so the browser never paints an
 * intermediate state — the original palette attr is restored before the next
 * frame. Returns null on the server where `document` is unavailable.
 */
function readPaletteSwatches(
    palettes: readonly (typeof PALETTES)[number][],
): Record<(typeof PALETTES)[number], string> | null {
    ...
    for (const p of palettes) {
        ...
        // If the CSS var read came up empty (e.g. stylesheet not yet
        // applied), fall back to the map matching the *current* `.dark`
        // class on <html> so the dot still looks right.
        const fallback = html.classList.contains("dark")
            ? PALETTE_SWATCH_FALLBACK_DARK
            : PALETTE_SWATCH_FALLBACK_LIGHT
        // `colorful` deliberately renders --primary as transparent, so the
        // OKLCH triple here is a red ghost we don't want in the picker.
        // Always use the gradient fallback for this palette.
        if (p === "colorful") {
            out[p] = fallback[p]
            continue
        }
    ...
    return out as Record<(typeof PALETTES)[number], string>
}
